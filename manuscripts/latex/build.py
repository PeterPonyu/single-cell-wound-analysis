#!/usr/bin/env python3
"""Compile the two native manuscripts, validate them, and publish separate PDFs.
...
NAMES = {1: "paper1_mixture_weight_construct_validation", 2: "paper2_population_flow_temporal_dynamics"}
...
def validate(n):
    folder = BUILD / f"paper{n}"
    stem = folder / f"paper{n}"
    ...
    with fitz.open(stem.with_suffix(".pdf")) as document:
        ...
        # Both manuscripts end in numerical supplements, so also reject a
...
def main():
    ...
    for n, name in NAMES.items():
        ...
        execute(["latexmk", "-norc", "-xelatex", "-bibtex", "-interaction=nonstopmode", "-halt-on-error",
                 "-file-line-error", "-no-shell-escape", f"-outdir={folder}", f"paper{n}.tex"],
                log=folder / "build.log")
        results[name] = validate(n)
    # Publish only after both documents pass; scientific reports are never edited.
    for n, name in NAMES.items():
        pdf = DEST / f"{name}.pdf"
        shutil.copy2(BUILD / f"paper{n}/paper{n}.pdf", pdf)
        results[name]["sha256"] = hashlib.sha256(pdf.read_bytes()).hexdigest()
        if args.render:
            review = BUILD / f"paper{n}/render"
