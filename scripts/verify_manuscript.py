...
PAPER_FILES = {'measurement_construct': MANUSCRIPTS / 'latex/paper1.tex', 'population_dynamics': MANUSCRIPTS / 'latex/paper2.tex'}
ANCHORS = {'measurement_construct': ['76,461', '243', '0.933', '0.841', '24.9%', '0.0280', '0.2719', '−0.0204', '561', '358', '0.909', '0.948', '0.714', '0.786', '0.937', '+0.0476', '0.0107', '0.0402', '0.302', '0.166', '1.66', '+0.210', '14.43/15', '0.0015', '5.0%'], 'population_dynamics': ['+49.9%', '−18.2%', '0.395', '0.5496', '0.3854', '0.156', '0.176', '0.368', '−1.5%', '+75.9%', '+0.948', '3/3', '6/6', '+32.1%', '+50.6%', '2/3', '0.165', '[0.081, 0.315]']}
...
def check_author_style() -> None:
    ...
    for n in (1, 2):
        ...
        main = (folder / f'paper{n}.tex').read_text()
        protected = {'abstract': main.split('\\section*{Abstract}', 1)[1].split('\\section{Introduction}', 1)[0], 'introduction': main.split('\\section{Introduction}', 1)[1].split('\\section{Results}', 1)[0], 'discussion': (folder / f'paper{n}_discussion.tex').read_text()}
        ...
        for section in ('methods', 'supplement'):
            text = (folder / f'paper{n}_{section}.tex').read_text()
            for label in re.finditer('\\\\label\\{(eq:[^}]+)\\}', text):
                ...
                totals['equations'] += 1
        narrative = '\n'.join(((folder / f'paper{n}_{section}.tex').read_text() for section in ('results', 'methods'))) + main
        tables = (folder / f'paper{n}_tables.tex').read_text() + (folder / f'paper{n}_supplement.tex').read_text()
        for label in re.findall('\\\\label\\{(tab:[^}]+)\\}', tables):
            ...
            totals['tables'] += 1
    print(f"PASS  author style: protected prose sections=6, adjacent equation citations={totals['equations']}, main-text table citations={totals['tables']}")
